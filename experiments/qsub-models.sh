...
set COUNTER = 0
set MAX = 80    # CHECK:  MAX <= 1/8 of number of CPUs requested
while ( $COUNTER < $MAX )
    ...
    # Run same seed for Model #1 and Model #2
    ...
    python driver.py --filename $OUTFILE --num_repeats $NUM_REPEATS $OBJECTIVE $DISTRIBUTION --seed $SEED -n $N_MIN $N_MAX $N_STEP -m $M_MIN $M_MAX $M_STEP --alternate-IP-model &
    @ COUNTER++
end
